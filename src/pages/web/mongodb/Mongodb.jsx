      ...
      <div>
        ...
        <ul className="list-inside list-disc">
          ...
          <li>
            ...
            merepresentasikan satu baris recordnya) sifatnya harus unik dan harus string sekalipun seluruhnya berupa nomor
            karena jika nomor diawali nol maka nol dianggap tidak ada.
          </li>
        ...
        <ul className="list-inside list-disc">
          <li>
            Mongodb adalah db dengan model noSql, di mana data disimpan dalam bentuk document mirip json (mongodb menyebutnya
            dengan istilah BSON)
          </li>
          <li>
            Document: menyimpan data dalam format document mirip dengan format JSON (javascript object notation). Setiap
            dokumen berisi pasangan field/kolom dan nilainya. nilai dapat berupa string, angka, boolean, array atau object.
            Dokumen database ini mengandung relasi antar data, berbagai tipe data serta SQL hingga membuatnya bisa
            dikomparasikan dengan relational db.
          </li>
          <li>
          ...
          <li>
            ...
            relasional karena setiap baris pada tabel tidak harus memiliki kolom yang sama. Db jenis ini bisa digunakan untuk
            menyimpan data pada data IoT (internet of thinks) dan data profil pengguna. Contoh db ini adalah: cassandra dan
            HBase
          </li>
          <li>
            Graph: menyimpan data dalam node dan edge. Node menyimpan info tentang orang, tempat dan apapun itu. Edge
            meyimpan info tentang hubungan masing masing node. Db jenis ini digunakan untuk menyimpan data model jaringan,
            fraud detection dan machine learning. Contohnya Neo4j dan JanusGraph
          </li>
          <li>
            MongoDB merupakan db dengan menggunakan model document base, yang menyimpan datanya dalam bentuk document mirip
            json
          </li>
          <li>
            perbandingan istilah sql db dan mogodb: table = collection, row = document/BSON document, column = field, table
            join = $lookup, embedded document. database = database, index = index, primary key = primary key
          </li>
        </ul>
...
new_db> db.products.deleteMany()`}</pre>
          <li>
            Authentication adalah mekanisme keamanan untuk membatasi hak akses pada db. Secara default kita masuk mongosh
            tanpa otentikasi, untuk aktifkan auth, kita harus tambahkan security pada konfigurasi mongo kita, caranya: cari
            file konfigurasi yakni tempat mongodb diunduh C:\Program Files\MongoDB\Server\6.0\bin, buka file mongod.cfg atau
            langsung search `mongod.cfg`. Lalu tambahkan berikut ini:
          </li>
          <pre>{`
security:
authorization: enabled`}</pre>
          ...
          <li>
            Role yang ada pada MongoDb: read, readWrite, dbAdmin, userAdmin, dbOwner, readAnyDatabase, userAdminAnyDatabase,
        ...
        <ul className="list-inside list-disc">
          <li>
            dibutuhkan driver. di npm ada driver yang dibuat mongodb bernama mongodb, ada juga library mongoose. Salahsatu
            manfaatnya yaitu memahami ODM (object document mapping) kalau di Sql itu ORM (object relational mapping). driver
            ini hanya perantara jadi dibutuhkan pengetahuan query native dan promise.
          </li>
          <li>
            mongoose adalah ODM (object data manager) paling populer mongoDB. mendukung pembuatan model dan validasi, custom
